...
public class WaypointEditor : Editor
{
    ...
    private void OnSceneGUI()
    {
        ...
        for (int i = 0; i < WaypointTarget.Points.Length; i++)
        {
            ...
            Vector3 currentPoint = WaypointTarget.EnityPosition +
            ...
            Handles.Label(WaypointTarget.EnityPosition + WaypointTarget.Points[i]
                + textPos, $"{i + 1}", text);

            if (EditorGUI.EndChangeCheck())
            {
                Undo.RecordObject(target, "Free Move");
                WaypointTarget.Points[i] = newPosition - WaypointTarget.EnityPosition;
            }
        }
    }
}
